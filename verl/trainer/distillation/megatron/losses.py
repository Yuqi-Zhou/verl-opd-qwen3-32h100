...
def _vocab_parallel_logsumexp_chunked(vp_logits: torch.Tensor, chunk_size: int = 16384) -> torch.Tensor:
    """Compute global logsumexp over vocab shards without materializing full log-probs."""
    from megatron.core.parallel_state import get_tensor_model_parallel_group

    tp_group = get_tensor_model_parallel_group()
    logits_max = vp_logits.max(dim=-1).values.float()
    ...
    for start in range(0, vp_logits.size(-1), chunk_size):
        logits_chunk = vp_logits[..., start : start + chunk_size].float()
        sum_exp += torch.exp(logits_chunk - logits_max.unsqueeze(dim=-1)).sum(dim=-1)

    torch.distributed.all_reduce(sum_exp, op=torch.distributed.ReduceOp.SUM, group=tp_group)
    return logits_max + sum_exp.log()


class _VocabParallelTopKOnlyKLDivergence(torch.autograd.Function):
    """Forward KL over teacher top-k ids without full-vocab log_softmax materialization."""
    ...
    def forward(
        ...
        tp_group = get_tensor_model_parallel_group()
        partition_vocab_size = vp_logits.size(-1)
        log_z = _vocab_parallel_logsumexp_chunked(vp_logits)
        ...
        target_topk_indices = target_topk_indices.clone() - vocab_start_index
        target_topk_indices[~topk_indices_in_vocab_mask] = 0
        ...
        topk = target_topk_indices.size(-1)
        vp_logits_2d = vp_logits.view(-1, partition_vocab_size)
        arange_1d = torch.arange(start=0, end=vp_logits_2d.size(0), device=vp_logits_2d.device)
        source_topk_logits_2d = vp_logits_2d[arange_1d.unsqueeze(-1), target_topk_indices.view(-1, topk)]
        source_topk_logps = source_topk_logits_2d.view(target_topk_indices.shape).float() - log_z.unsqueeze(-1)

        active_mask = topk_indices_in_vocab_mask
        if log_prob_min_clamp is not None:
            active_mask = active_mask & (source_topk_logps > log_prob_min_clamp)
            source_topk_logps = source_topk_logps.clamp_min(log_prob_min_clamp)
            target_active_probs = target_topk_probs.clone()
            target_active_probs[~active_mask] = 0
            target_active_mass = target_active_probs.sum(dim=-1)
            torch.distributed.all_reduce(target_active_mass, op=torch.distributed.ReduceOp.SUM, group=tp_group)
        else:
            target_active_mass = target_topk_mass

        source_topk_logps[~topk_indices_in_vocab_mask] = 0

        per_token_kl_loss = torch.sum(
            target_topk_probs * (target_topk_logps - source_topk_logps),
            dim=-1,
        )
        torch.distributed.all_reduce(per_token_kl_loss, op=torch.distributed.ReduceOp.SUM, group=tp_group)

        # Megatron may mutate logits after the logits processor returns. Save an
        # independent, reduced-precision copy for backward recomputation.
        saved_vp_logits = vp_logits.detach()
        if saved_vp_logits.dtype == torch.float32:
            saved_vp_logits = saved_vp_logits.to(torch.bfloat16)
        else:
            saved_vp_logits = saved_vp_logits.clone()
        ctx.input_dtype = vp_logits.dtype
        ctx.save_for_backward(
            saved_vp_logits, log_z, target_topk_probs, target_topk_indices, active_mask, target_active_mass
        )

        source_topk_probs = source_topk_logps.exp() * topk_indices_in_vocab_mask
        per_token_topk_mass = torch.sum(source_topk_probs, dim=-1)
        torch.distributed.all_reduce(per_token_topk_mass, op=torch.distributed.ReduceOp.SUM, group=tp_group)

        local_topk = min(topk, partition_vocab_size)
        local_student_topk_logits, local_student_topk_ids = torch.topk(vp_logits, k=local_topk, dim=-1)
        local_student_topk_logps = local_student_topk_logits.float() - log_z.unsqueeze(-1)
        local_student_topk_ids = local_student_topk_ids + vocab_start_index
        gathered_student_topk_logps = [torch.empty_like(local_student_topk_logps) for _ in range(world_size)]
        gathered_student_topk_ids = [torch.empty_like(local_student_topk_ids) for _ in range(world_size)]
        torch.distributed.all_gather(gathered_student_topk_logps, local_student_topk_logps, group=tp_group)
        torch.distributed.all_gather(gathered_student_topk_ids, local_student_topk_ids, group=tp_group)
        student_topk_logps = torch.cat(gathered_student_topk_logps, dim=-1)
        student_topk_ids = torch.cat(gathered_student_topk_ids, dim=-1)
        _, student_topk_positions = torch.topk(student_topk_logps, k=topk, dim=-1)
        student_topk_ids = torch.gather(student_topk_ids, dim=-1, index=student_topk_positions)

        per_target_token_kl = target_topk_probs * (target_topk_logps - source_topk_logps)
        torch.distributed.all_reduce(per_target_token_kl, op=torch.distributed.ReduceOp.SUM, group=tp_group)
    ...
    def backward(
        ...
        grad_input = torch.empty(saved_vp_logits.shape, device=saved_vp_logits.device, dtype=ctx.input_dtype)
        scale = (target_active_mass * grad_loss).float()
        scale = scale.unsqueeze(dim=-1)
        for start in range(0, saved_vp_logits.size(-1), 4096):
            end = start + 4096
            probs_chunk = saved_vp_logits[..., start:end].float()
            probs_chunk.sub_(log_z.unsqueeze(dim=-1)).exp_().mul_(scale)
            grad_input[..., start:end].copy_(probs_chunk)

        topk = target_topk_indices.size(-1)
        grad_input_2d = grad_input.view(-1, grad_input.size(-1))
        target_topk_probs_flat = target_topk_probs.view(-1, topk)
        target_topk_indices_flat = target_topk_indices.view(-1, topk)
        sub = (
            target_topk_probs_flat
            * active_mask.view(-1, topk).to(target_topk_probs_flat.dtype)
            * grad_loss.reshape(-1, 1).float()
        )
        grad_input_2d.scatter_add_(dim=1, index=target_topk_indices_flat, src=-sub.to(grad_input_2d.dtype))
        return grad_input, None, None, None
...
def compute_forward_kl_topk(
    ...
    response_mask_cp_split = None
    if response_mask is not None:
        try:
            if response_mask.is_nested:
                response_mask_for_preprocess = response_mask.bool().unsqueeze(-1)
                if data_format == "thd":
                    response_mask_cp_split, *_ = preprocess_thd_engine(response_mask_for_preprocess, pre_process=True)
                else:
                    response_mask_cp_split, *_ = preprocess_bshd_engine(response_mask_for_preprocess, pre_process=True)
                response_mask_cp_split = response_mask_cp_split.squeeze(-1).bool()
            else:
                response_mask_cp_split = response_mask.bool()
        except Exception:
            response_mask_cp_split = None
    ...
    # The final distillation loss is aggregated with response_mask, so prompt-token
    # KL values do not affect the objective. Avoid full-vocab softmax over prompt
    # tokens for long-context OPD; this cuts the peak memory in this loss by roughly
    # prompt+response over response length.
    if (
        response_mask_cp_split is not None
        and response_mask_cp_split.shape == student_logits.shape[:2]
        and response_mask_cp_split.any()
    ):
        selected_student_logits = student_logits[response_mask_cp_split].unsqueeze(0)
        selected_teacher_log_probs = teacher_topk_log_probs_cp_split[response_mask_cp_split].unsqueeze(0)
        selected_teacher_ids = teacher_topk_ids_cp_split[response_mask_cp_split].unsqueeze(0)

        selected_outputs = _VocabParallelTopKOnlyKLDivergence.apply(
        ...
        def scatter_response_only(selected: torch.Tensor) -> torch.Tensor:
            output = selected.new_zeros(student_logits.shape[:2])
            return output.masked_scatter(response_mask_cp_split, selected.reshape(-1))

        distillation_losses, student_mass, teacher_mass, overlap_count, overlap_token_advantage = (
            scatter_response_only(selected_outputs[0]),
            scatter_response_only(selected_outputs[1]),
            scatter_response_only(selected_outputs[2]),
            scatter_response_only(selected_outputs[3]),
            scatter_response_only(selected_outputs[4]),
        )
    ...
    distillation_losses, student_mass, teacher_mass, overlap_count, overlap_token_advantage = (
        _VocabParallelTopKOnlyKLDivergence.apply(
            student_logits,
